  ...
  const filteredData = useMemo(() => {
    return fakeData.filter(
      (card) =>
        ...
        `${card.question.question_id}. ${card.question.translatedTitle}`.includes(search)
    );
  }, [search]);

  // Get the paginated data
  useEffect(() => {
    setPaginatedData(filteredData.slice(page * rowCount, (page + 1) * rowCount));
    ...
    <Container maxW="container.xl">
      ...
      >
        <Stack spacing="5">
          ...
          {/* Main Content */}
          <Box overflowX="auto">
            <QuestionTable search={search} cards={paginatedData} rowCount={rowCount} />
          </Box>
          {/* Footer */}
            ...
            <HStack spacing="3" justify="space-between">
              {!isMobile && (
                <Text color="muted" fontSize="sm">
                  {`Showing ${page * rowCount + 1} to ${(page + 1) * rowCount} of ${
                    filteredData.length
                  } results`}{' '}
                </Text>
              )}
              ...
              >
                ...
                <Button
                  variant="solid"
                  onClick={() => {
                    setPage(Math.min(Math.floor(filteredData.length / rowCount), page + 1));
                  }}
                >
                  Next
                </Button>
