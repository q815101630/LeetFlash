import { DefaultUserPerformance } from "../background/background";
...
export interface UserPerformance {
  ...
  num_medium: number;
  num_hard: number;
  finishedEasy: Set<string>;
  finishedMedium: Set<string>;
  finishedHard: Set<string>;
}
  ...
  return new Promise((resolve, reject) => {
    getStoredUser().then((user) => {
      ...
      if (question_type === "easy") {
        performance.finishedEasy.add(question_id);
      } else if (question_type === "medium") {
        performance.finishedMedium.add(question_id);
      } else if (question_type === "hard") {
        performance.finishedHard.add(question_id);
      }
      setStoredUser(user)
        .then(() => resolve())
        .catch((err) => reject(err));
